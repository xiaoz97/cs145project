...
def getFrequentPatterns():
    ...
    # 给u.data 加标题行
    all_ratings = pd.read_csv(ratings_filename,header=1)
...
favorable_ratings = all_ratings[all_ratings["rating"] == 1]
dislike_ratings=all_ratings[all_ratings['rating']==0]
candidate_rules = []
...
dislike_reviews_by_users=dict(
        (k, frozenset(v.values)) for k, v in dislike_ratings.groupby("userId")["movieId"])
...
if os.path.isfile(os.path.join(data_folder, "sorted_confidence.npy")):
    ...
    for user, reviews in favorable_reviews_by_users.items():
        # 遍历每条关联规则
        #print(dislike_reviews_by_users[user])
        ...
        for candidate_rule in candidate_rules:
            ...
            if set(premise).issubset(reviews):
                ...
                if conclusion in reviews:
                    ...
                    # print(conclusion)
                    # print(reviews)
                    # print("correct"+ str(correct_counts[candidate_rule]))
                if user in dislike_reviews_by_users:
                    if conclusion in dislike_reviews_by_users[user]:
                        # print(conclusion)
                        # print(dislike_reviews_by_users[user])
                        incorrect_counts[candidate_rule] += 1
                        # print("incorrect"+ str(incorrect_counts[candidate_rule]))
                        # print(reviews)
                        # print(user)
                        # exit()
...
for index, row in validate.iterrows():
    ...
    for confidence, rules in sorted_confidence:
        ...
        if rules[1] == row["movieId"]:
            if row["movieId"] in favorable_reviews_by_users and (len(list(rules[0]) + list(favorable_reviews_by_users[row["userId"]])) - len(
